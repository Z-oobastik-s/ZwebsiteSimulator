...
// Theme toggle — плавная смена темы без лагов
function toggleTheme() {
    ...
    // Иконка: лёгкое вращение и смена символа
    const icon = DOM.get('themeIcon');
    if (icon) {
        icon.style.opacity = '0.5';
        icon.style.transform = 'rotate(-15deg) scale(0.95)';
    ...
    // Фон и футер меняем после завершения перехода цветов — без резкого переключения
    setTimeout(() => {
        setRandomBackground();
        updateFooterBackground();
